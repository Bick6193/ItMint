...
selOpt.attr("class"):"");optList.find("li").removeClass("selected");optList.find("li").eq(i).addClass("selected")}});if(optList.css("overflow")!="hidden"){if(optList.scrollTop()>optList.find("li.selected").position().top+optList.scrollTop())optList.scrollTop(optList.find("li.selected").position().top+optList.scrollTop());if(optList.scrollTop()+optList.height()<optList.find("li.selected").position().top+optList.find("li.selected").outerHeight()+optList.scrollTop())optList.scrollTop(optList.find("li.selected").position().top+
optList.scrollTop()-optList.height()+optList.find("li.selected").outerHeight())}if((e.keyCode==13||e.keyCode==27)&&replaced.hasClass(options.hoverClass)){replaced.removeClass(options.hoverClass);optHolder.css({left:-9999,top:-9999})}else if(e.keyCode==13&&!replaced.hasClass(options.hoverClass))openDrop()};select.bind("change.customForm",changeSelect);select.get(0).onkeypress=changeSelect;select.bind("focus.customForm",function(){replaced.addClass("focus");select.attr("size",4)}).bind("blur.customForm",
function(){replaced.removeClass("focus");select.removeAttr("size")});select.data("customForm",{"resizeElement":function(){if(time)clearTimeout(time);time=setTimeout(function(){var w=0;select.removeClass("outtaHere");$("div.selectArea.customSelect").css({display:"none"});w=Math.floor(select.outerWidth());select.addClass("outtaHere");$("div.selectArea.customSelect").css({display:"block"});replaced.width(w)},300)},"create":replaced.add(optHolder)});$(window).bind("resize.customForm",function(){if(select.data("customForm")["resizeElement"])select.data("customForm").resizeElement()})}).addClass("outtaHere")}
